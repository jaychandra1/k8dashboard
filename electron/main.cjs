...
const ENV_PREFIXES = ['LLM_', 'MCP_', 'AWS_', 'AZURE_', 'GOOGLE_', 'KUBE'];
  ...
  serverProcess.on('exit', (code) => {
    ...
    if (!app.isQuitting && code !== 0 && code !== null) {
      ...
      const detail = portTaken
        ? `Port ${port} was taken by another process while k8sight was starting. Relaunch the app.`
...
async function boot() {
  ...
  if (backendPort !== PREFERRED_PORT) {
    console.warn(`[k8sight] port ${PREFERRED_PORT} is busy; backend will use ${backendPort}`);
  }
