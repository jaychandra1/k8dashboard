...
import { HttpError, KUBECTL_REQUIRED_MESSAGE } from './http-errors.mjs';

const IS_WIN = process.platform === 'win32';

// KUBEPILOT_KUBECTL_BIN pins the kubectl binary (absolute path). Tests use it
// to simulate a machine without kubectl (`/nonexistent`); it is also handy when
// the binary lives somewhere the PATH search below would not look.
const kubectlOverride = (cmd) => (cmd === 'kubectl' && process.env.KUBEPILOT_KUBECTL_BIN ? String(process.env.KUBEPILOT_KUBECTL_BIN) : null);
...
export const resolveBinSync = (cmd) => {
  ...
  const override = kubectlOverride(safe);
...
export const commandExists = async (cmd) => {
  ...
  const override = kubectlOverride(safe);
  if (override) return isExecutable(override);
...
// ------------------------------------------------------------------
// kubectl is a hard requirement only for the interactive pod terminal and for
// service port-forward; every other read/mutation goes through the Kubernetes
// API. KUBECTL_REQUIRED_MESSAGE (lib/http-errors.mjs) is what those two
// features answer when it is missing.
export { KUBECTL_REQUIRED_MESSAGE };

// 501 { error, code: 'kubectl_required' } — an HttpError, so mapUpstreamError
// and the error middleware render it as-is.
export const kubectlMissingError = () => new HttpError(501, KUBECTL_REQUIRED_MESSAGE, 'kubectl_required');

// A child_process spawn failure for a binary that does not exist.
export const isSpawnMissing = (err) =>
  !!err && err.code === 'ENOENT' && (String(err.syscall || '').startsWith('spawn') || /^spawn\b/.test(String(err.message || '')));

// { available, path } for kubectl, memoized for 60 s; `refresh: true` forces a
// re-check (the capabilities endpoint calls it on demand).
let kubectlProbe; // { at, value }
export const hasKubectl = async ({ refresh = false } = {}) => {
  if (!refresh && kubectlProbe && Date.now() - kubectlProbe.at < 60_000) return kubectlProbe.value;
  await loginShellPath(); // warm the login-shell PATH so resolveBinSync sees it
  if (refresh) resolveCache.delete('kubectl');
  const resolved = resolveBinSync('kubectl');
  const available = resolved !== 'kubectl' && isExecutable(resolved);
  const value = { available, path: available ? resolved : null };
  kubectlProbe = { at: Date.now(), value };
  return value;
};
...
export function createKubectl({ getContext, bin = 'kubectl' } = {}) {
  ...
  // A missing binary surfaces as the friendly kubectl_required error, never as
  // a raw `spawn kubectl ENOENT`.
  const execKubectl = async (args, { input, timeoutMs = 25000, maxBuffer = 10 * 1024 * 1024, requestTimeout } = {}) => {
    ...
    try {
      ...
      return await collectChild(child, { input, timeoutMs, maxBuffer, label: 'kubectl' });
    } catch (e) {
      if (isSpawnMissing(e)) throw kubectlMissingError();
      throw e;
    }
  };
