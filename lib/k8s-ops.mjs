...
// ------------------------------------------------------------------
// Label selectors (pure)
// ------------------------------------------------------------------
// A workload's `spec.selector` (matchLabels + matchExpressions) as the
// `labelSelector` query string the list APIs take — what `kubectl logs
// deploy/x` / `kubectl get pods -l` use to find the workload's pods. Returns
// '' for an empty selector; callers must treat that as "no pods", never as
// "every pod in the namespace".
export function labelSelectorFor(selector) {
