...
// Hidden test/dev fixture: KUBEPILOT_DEMO=1 lets server.js offer the synthetic
// 'demo-cluster' from demo.js. Off by default and never forwarded by the
// packaged app (electron/main.cjs strips it), so end users only ever see real
// clusters. Deliberately not envFlag(): there are no legacy names for this.
export const DEMO_ENABLED = /^(1|true|yes)$/i.test(String(process.env.KUBEPILOT_DEMO || ''));
