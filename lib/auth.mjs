...
//
// Token source order: K8SIGHT_TOKEN env → ~/.config/k8sight/token → generated
...
const TOKEN_DIR = path.join(os.homedir(), '.config', 'k8sight');
...
export function getAuthToken() {
  ...
  let token = String(process.env.K8SIGHT_TOKEN || '').trim();
  if (!token) {
    try { token = fs.readFileSync(TOKEN_FILE, 'utf8').trim(); } catch { token = ''; }
