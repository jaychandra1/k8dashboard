...
describe('token storage', () => {
  it('stores in sessionStorage and memory', () => {
    ...
    expect(sessionStorage.getItem('kubepilot.token')).toBe('abc');
    clearToken();
    expect(getToken()).toBeNull();
  });
  it('reads a legacy k8dashboard.token and migrates it on the next write', () => {
    sessionStorage.setItem('k8dashboard.token', 'old-tok');
    expect(getToken()).toBe('old-tok');
    setToken('new-tok');
    expect(sessionStorage.getItem('kubepilot.token')).toBe('new-tok');
    expect(sessionStorage.getItem('k8dashboard.token')).toBeNull();
  });
...
describe('axios instance', () => {
  it('adds Authorization + X-Requested-With headers', async () => {
    ...
    expect(cfg.headers['X-Requested-With']).toBe('kubepilot');
    spy.mockRestore();
  });
...
describe('sseFetch', () => {
  it('adds headers and throws ApiError on non-2xx', async () => {
    ...
    expect(init.headers['X-Requested-With']).toBe('kubepilot');
