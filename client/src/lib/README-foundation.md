...
- `useHashRoute()` → `{ route: { view, params[], query{} }, hash, navigate(view, params?, query?, { replace }), back, forward, canBack, canForward, setQuery(patch), isSame(route) }`. Pure helpers `parseHash(hash)`, `buildHash(view, params, query)`; `navigateTo()` for non-React code.
  **App.jsx adoption (App owner):** replace `resourceType` state + the `history` stack with `const { route, navigate, back, forward, canBack, canForward } = useHashRoute()`; `resourceType = route.view`; `setResourceType(k)` → `navigate(k)`; the Back/Forward buttons (`back`/`canBack`) and the `ClusterSwitcher variant="sidebar"` are rendered by App as one `.nav-cluster-row` and passed to `Navigation` via `headerExtra`; TopBar gets only `crumb` (page breadcrumb) + its controls; drawer selection → `navigate(route.view, [ns, name])`; namespace filter → `setQuery({ ns })`; Preferences → `navigate('preferences', [section])` (it's `hidden` in the registry so it stays out of nav). Views that own sub-views (argocd/security) use `route.params[0]`.
