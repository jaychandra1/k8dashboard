...
| --- | --- | --- |
| `api` | axios instance (`baseURL: ''`) | adds `Authorization: Bearer <token>` + `X-Requested-With: k8sight`; rejects with `ApiError` |
...
| `bootstrapTokenFromHash()` | → bool | called in `main.jsx`; reads `#token=` / `#/view?token=` into `sessionStorage['k8sight.token']` and scrubs the hash |
...
- `useDocumentTitle('Pods')` → `Pods · k8sight`, restored on unmount.
