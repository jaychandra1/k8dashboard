...
export function handle(req, res) {
  try {
    ...
    // Route words are matched case-insensitively (`p` / `sl`); path parameters
    // keep their original spelling (`seg`).
    ...
    if (p.startsWith('/api/config') || p.startsWith('/api/azure') || p.startsWith('/api/aws') ||
        ...
        p === '/api/version' || p.startsWith('/api/mcp')) {
      return false;
    }
    if (!p.startsWith('/api/')) return false;

    const seg = url.pathname.split('/').filter(Boolean); // e.g. ['api','resources','shop']
    const sl = seg.map((s) => s.toLowerCase());          // lowercased, for route-word matching
    const json = (obj, code = 200) => { res.status(code).json(obj); return true; };

    // ---------- exec (kubectl passthrough) ----------
    if (method === 'POST' && p === '/api/exec') {
      const command = typeof req.body?.command === 'string' ? req.body.command.slice(0, 4096) : '';
      return json({ output: `(demo) ${command}\n`, code: 0 });
    }
    ...
    if (method === 'GET' && sl[1] === 'nodes' && sl[3] === 'pods') {
    ...
    // ---------- resources list ----------
    if (method === 'GET' && sl[1] === 'resources' && seg[2]) {
    ...
    // ---------- resource detail ----------
    if (method === 'GET' && sl[1] === 'resource' && seg.length === 5) {
    ...
    // ---------- resource delete ----------
    if (method === 'DELETE' && sl[1] === 'resource' && seg.length === 5) {
      const [, , nsp, kind, name] = seg.map(decodeURIComponent);
      return json(deleteResource(nsp, kind, name));
    }

    // ---------- yaml get / put ----------
    if (sl[1] === 'yaml' && seg.length === 5) {
      const [, , nsp, kind, name] = seg.map(decodeURIComponent);
      if (method === 'GET') {
        const obj = findResource(nsp, kind, name);
        if (!obj) return json({ error: 'Resource not found' }, 404);
    ...
    // ---------- scale ----------
    if (method === 'POST' && sl[1] === 'scale' && seg.length === 5) {
      const [, , nsp, kind, name] = seg.map(decodeURIComponent);
    ...
    // ---------- restart ----------
    if (method === 'POST' && sl[1] === 'restart' && seg.length === 5) {
      const [, , nsp, kind, name] = seg.map(decodeURIComponent);
    ...
    if (method === 'GET' && sl[1] === 'helm' && sl[2] === 'releases' && sl[5] === 'values') {
    ...
    if (method === 'GET' && sl[1] === 'helm' && sl[2] === 'releases' && sl[5] === 'manifest') {
      const rel = cluster.helm.find((r) => r.namespace === decodeURIComponent(seg[3]) && r.name === decodeURIComponent(seg[4]));
      if (!rel) return json({ error: 'Release not found' }, 404);
    ...
    if (method === 'GET' && sl[1] === 'customresources' && seg.length === 5) {
    ...
    if (method === 'GET' && sl[1] === 'customresource' && seg.length === 6) {
      const key = `${decodeURIComponent(seg[2])}/${decodeURIComponent(seg[3])}/${decodeURIComponent(seg[4])}`;
    ...
    if (method === 'GET' && sl[1] === 'argocd' && sl[2] === 'application' && seg.length === 5) {
      return json(argoAppDetail(decodeURIComponent(seg[3]), decodeURIComponent(seg[4])));
    }
    if (method === 'POST' && sl[1] === 'argocd' && sl[2] === 'application' && sl[5] === 'sync') {
    ...
    if (method === 'POST' && sl[1] === 'argocd' && sl[2] === 'application' && sl[5] === 'refresh') {
      const app = cluster.argo.apps.find((a) => a.metadata.namespace === decodeURIComponent(seg[3]) && a.metadata.name === decodeURIComponent(seg[4]));
      if (app) app.status.reconciledAt = nowISO();
      return json({ success: true, message: 'Refresh requested' });
    }
    if (method === 'DELETE' && sl[1] === 'argocd' && sl[2] === 'application' && seg.length === 5) {
    ...
    // ---------- topology ----------
    if (method === 'GET' && sl[1] === 'topology' && seg[2]) return json(buildTopology(decodeURIComponent(seg[2])));
