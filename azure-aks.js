...
export async function startBrowserLogin(rawTenant = 'organizations') {
  ...
  flow = { server, verifier, state, redirectUri, status: 'pending', error: null, tenant, authUrl: null, timer: null };

  // Auto-cancel: an abandoned sign-in must not leave a listener open forever.
  const thisFlow = flow;
  thisFlow.timer = setTimeout(() => {
    if (flow !== thisFlow || thisFlow.status !== 'pending') return;
    thisFlow.status = 'error';
    thisFlow.error = 'Sign-in timed out after 10 minutes — start again.';
    closeFlowServer();
  }, LOGIN_TIMEOUT_MS);
  thisFlow.timer.unref?.();
...
export async function listAllClusters() {
  ...
  const perSub = await Promise.all(subs.map(async (s) => {
    try {
      const list = await armList(`/subscriptions/${encodeURIComponent(s.id)}/providers/Microsoft.ContainerService/managedClusters?api-version=${API_AKS}`);
      return list.map((a) => ({
        name: a.name,
...
export async function getClusterKubeconfig(subscriptionId, resourceGroup, name, admin = false) {
  ...
  const j = await arm(
    `${ARM}/subscriptions/${seg(subscriptionId)}/resourceGroups/${seg(resourceGroup)}/providers/Microsoft.ContainerService/managedClusters/${seg(name)}/${action}?api-version=${API_AKS}`,
    { method: 'POST' },
  );
