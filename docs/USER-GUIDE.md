...
Current version: `1.5.1` (see the `VERSION` file). Node.js **22 or newer** is required to build or run from source.
...
Download the installer for your OS from the GitHub Releases page (`k8sight-macos.dmg` for Apple Silicon, `k8sight-windows.exe`, `k8sight-linux.AppImage` or `.deb`) and open it.
...
```
k8sight listening on http://127.0.0.1:3001  —  open http://127.0.0.1:3001/#token=<token>
```
...
The token is generated once and saved to `~/.config/k8sight/token` (mode `0600`), so it stays the same across restarts unless you set `K8SIGHT_TOKEN` yourself.
...
Then open `http://localhost:8080/#token=<token>`. To use a fixed token instead, pass `-e K8SIGHT_TOKEN=<at least 16 characters>`.
...
```bash
claude mcp add --transport http k8sight http://127.0.0.1:3001/mcp \
...
Imported contexts are merged into your kubeconfig. The existing file is **backed up** (`config.k8sight-backup-<timestamp>`) before the first write of a session, written atomically with mode `0600`, and never overwritten if it cannot be parsed. Imported clusters authenticate at runtime through bundled token helpers, so `aws`, `az` and `kubelogin` are not needed afterwards.

### 1.9 Configuration reference

| Variable | Purpose | Default |
|---|---|---|
| `K8SIGHT_TOKEN` | Bearer token for `/api/*`, `/mcp` and `/ws/exec` | generated → `~/.config/k8sight/token` |
...
| `ALLOWED_HOSTS` | Extra `Host` values accepted (e.g. `k8sight.internal:8080`) | — |
...
App data lives in `~/.config/k8sight` (token, assistant config, scan cache). Logs are JSON lines on stdout; the request log never contains query strings, headers or tokens.
...
Versions follow **Semantic Versioning**: patch for fixes (`1.5.1 → 1.5.2`), minor for features (`→ 1.6.0`), major for breaking changes (`→ 2.0.0`). The single source of truth is the `VERSION` file; everything else is derived from it.
...
2. **Bump the version everywhere with one command:**
   ...
   ```bash
   node scripts/sync-version.mjs 1.6.0
   ```
...
3. **Update `CHANGELOG.md`.** Rename the `## [Unreleased]` section to `## [1.6.0] – 2026-MM-DD` and start a fresh empty `Unreleased` above it. Keep the Keep-a-Changelog headings (Added / Changed / Fixed / Security).
...
5. **Watch the Build & Release workflow** (GitHub → Actions → *Build & Release*). It runs these jobs in order:
   ...
   | **Publish Docker image (GHCR)** | Also gated by the `release` environment. Builds amd64 + arm64 and pushes `ghcr.io/praveenraghav01/k8sight:v1.6.0`, `:1.6` and `:latest`. |
...
6. **Verify the release.** Download one installer and check it against `SHA256SUMS.txt`; pull the image and hit `/healthz`:
   ...
   docker run --rm -p 127.0.0.1:8080:3001 ghcr.io/praveenraghav01/k8sight:v1.6.0
...
Outputs in `release/`: `k8sight-macos.dmg` (Apple Silicon), `k8sight-windows.exe` (NSIS), `k8sight-linux.AppImage` and `k8sight-linux.deb`. Packaging for macOS requires a Mac; Windows requires Windows (or Wine), Linux requires Linux. Builds are ad-hoc signed; to notarize, obtain a Developer ID, set `hardenedRuntime: true` in `package.json` (the entitlements file is already in `build/`) and provide signing credentials.

### 3.3 Building the Docker image locally

```bash
docker build -t k8sight:dev .
docker run --rm -p 127.0.0.1:8080:3001 -e K8SIGHT_TOKEN=local-dev-token-1234 \
  -v "$HOME/.kube:/home/node/.kube:ro" k8sight:dev
```
...
1. Branch from the release tag: `git checkout -b hotfix/1.6.1 v1.6.0`.
2. Fix, add a test, update `CHANGELOG.md`.
3. `node scripts/sync-version.mjs 1.6.1`, commit, open a PR to `main`, merge.
4. Tag the merged commit `v1.6.1` and push the tag; the workflow does the rest.
