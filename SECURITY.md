...
k8sight runs with the full privileges of your kubeconfig. We treat anything that lets an
...
- Preferred: open a private report via
  [GitHub Security Advisories](https://github.com/praveenraghav01/k8sight/security/advisories/new).
- Or email **security@k8sight.in** (placeholder — replace with the maintainer's monitored address).
...
Windows, Linux and as `ghcr.io/praveenraghav01/k8sight`.
...
- Vulnerabilities in the clusters you connect to, or in third-party tools (`kubectl`, `trivy`)
  unless k8sight invokes them unsafely.
- The marketing website (static HTML) beyond content injection.

## Hardening summary (what the app does)

- Bearer token (`K8SIGHT_TOKEN` or `~/.config/k8sight/token`, mode 0600) on every non-public route.
