...
// `Authorization: Bearer <token>` (MCP_API_TOKEN, else K8SIGHT_TOKEN, else
// ~/.config/k8sight/token) and `X-K8sight-Source: mcp`.
//
...
if (!token) console.error('[k8sight MCP] no API token found (set MCP_API_TOKEN / K8SIGHT_TOKEN or start the app once to create ~/.config/k8sight/token)');
...
console.error('[k8sight MCP] stdio server ready');
